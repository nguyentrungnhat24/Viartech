  ...
  <div id="id01" class="modal" style="width: 100%;height:100%">
    <form style="width:800px;margin-left:300px;background:white;padding:10px" action="controllers.php?act=khachhang_add" method="post" enctype="multipart/form-data">
      ...
      <div class="container ">
        ...
        <select name="iddmgoitap" id="">
          <option value=0>Chọn gói tập</option>
          <?php
          $iddmgtcur = $khct[0]['iddmgoitap'];
          if (isset($dsdmgt)) {
            foreach ($dsdmgt as $dmgt) {
              if ($dmgt["id"] == $iddmgtcur)
                echo '<option value = "' . $dmgt['id'] . '" selected>' . $dmgt['tendmgoitap'] . '</option>';
              else
                echo '<option value = "' . $dmgt['id'] . '">' . $dmgt['tendmgoitap'] . '</option>';
            }
          }
          ?>
        </select>
      ...
      <div class="container" style="background-color:#f1f1f1">
        <input class="btn btn-success p-2 mb-2" type="submit" name="themmoikh" value="Đăng kí">
      ...
      <div class="list1">
        <form class="modal-content animate" action="controllers.php?act=khachhang_update" method="post" enctype="multipart/form-data">
          ...
          <div class="container">
            ...
            <select name="idpt" id="">
              <option value=0>Chọn PT</option>
              <?php
              $iddmptcur = $khct[0]['idpt'];
              if (isset($dsdmpt)) {
                foreach ($dsdmpt as $dmpt) {
                  if ($dmpt["id"] == $iddmptcur)
                    echo '<option value = "' . $dmpt['id'] . '" selected>' . $dmpt['tenpt'] . '</option>';
                  else
                    echo '<option value = "' . $dmpt['id'] . '">' . $dmpt['tenpt'] . '</option>';
                }
              }
              ?>
            </select>
            <label for="uname"><b>Tên khách hàng</b></label>
            <input type="text" name="tenkh" value="<?= $khct[0]['tenkh'] ?>" required>

            <label for="psw"><b>Hình ảnh</b></label>
            <input type="file" name="image">
            <!-- <img src="<?= $khct[0]['image'] ?>" width="50"> -->
            <label for="uname"><b>Số điện thoại</b></label>
            <input type="number" name="sodt" value="<?= $khct[0]['sodt'] ?>" required>
            <label for="uname"><b>Email</b></label>
            <input type="text" name="email" value="<?= $khct[0]['email'] ?>" required id="txtEmail" aria-describedby="msgEmail">
            <small id="msgEmail" class="text-muted" style="color:red !important;display:none">
            </small><br>
            <label for="uname"><b>Địa chỉ</b></label>
            <input type="text" name="diachi" value="<?= $khct[0]['diachi'] ?>" required>
            <input type=hidden name="id" value="<?= $khct[0]['id'] ?>">
